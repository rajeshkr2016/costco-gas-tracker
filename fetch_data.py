...
Reads config.json for the list of ZIPs to track, fetches gas
prices for each via Costco's warehouse locator, and writes static JSON files
into pwa/data/ for the PWA to read.
...
def geocode_zip(zip_code: str, cache: dict) -> tuple[float, float, str]:
    ...
    # zip_cache.json manually (one entry per ZIP).
    try:
        ...
        raise RuntimeError(
            ...
            f"Add an entry to zip_cache.json: "
